...
- Use explicit versioned SQL migrations through
  `github.com/golang-migrate/migrate/v4`.
- Use PostgreSQL `pgcrypto` and `gen_random_uuid()` for UUID defaults.
...
The initial users, organizations, and organization membership schema is
implemented. Passwords, authentication, JWT, registration, login, products,
warehouses, inventory, orders, Redis, and workers remain future work.
